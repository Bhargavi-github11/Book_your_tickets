...
const Theaters = () => {
  ...
  const fetchTheatersPage = async (page) => {
    try {
      setLoading(true)
      ...
      const { data } = await axios.get(`/api/show/theaters?${params.toString()}`)
  ...
  const onNext = () => {
    if (currentPage < totalPages) {
      fetchTheatersPage(currentPage + 1)
      scrollTo(0, 0)
    }
  }
  ...
  return movies.length > 0 ? (
    <div className='relative my-40 mb-60 px-6 md:px-16 lg:px-40 xl:px-44 overflow-hidden min-h-[80vh]'>
      <BlurCircle top='150px' left='0px' />
      <BlurCircle bottom='50px' right='50px' />
      <h1 className='text-lg font-medium my-4'>In Theaters</h1>
    ...
    <div className='flex flex-col items-center justify-center h-screen'>
      <h1 className='text-3xl font-bold text-center'>No theater movies available</h1>
    </div>
  )
}
